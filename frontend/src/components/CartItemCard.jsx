...
function CartItemCard({ data }) {
    ...
    return (
        <div className='flex items-center justify-between bg-white p-4 rounded-xl shadow border hover:shadow-md transition-shadow'>
            <div className='flex items-center gap-4'>
                ...
                <div>
                    ...
                    <p className="font-bold text-[#ff4d2d]">₹{data.price * data.quantity}</p>
                </div>
            </div>
            <div className='flex items-center gap-3'>
                <div className='flex items-center bg-gray-50 rounded-full border border-gray-100 p-1'>
                    <button className='p-2 cursor-pointer bg-white rounded-full hover:bg-gray-100 shadow-sm transition-colors' onClick={() => handleDecrease(data.id, data.quantity)}>
                        <FaMinus size={10} />
                    </button>
                    <span className='w-8 text-center font-bold text-sm'>{data.quantity}</span>
                    <button className='p-2 cursor-pointer bg-white rounded-full hover:bg-gray-100 shadow-sm transition-colors' onClick={() => handleIncrease(data.id, data.quantity)}>
                        <FaPlus size={10} />
                    </button>
                </div>
                <button className="p-2.5 bg-red-50 text-red-500 rounded-full hover:bg-red-500 hover:text-white transition-all duration-300"
